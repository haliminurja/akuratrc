        ...
        <div class="card mb-5 mb-xl-8 border-2 shadow p-3 mb-5 bg-white rounded">
            <div class="card-header">
                ...
                <div class="card-toolbar">
                    ...
                    <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        <a type="button" class="btn btn-sm btn-primary" href="{{ route('admin.visi.create') }}">Tambah</a>
                    </div>
                </div>
            ...
            <div class="card-body">
                <div class="text-muted mb-3">
                    Pada halaman ini digunakan untuk menambah, mengedit, dan detail visi
                </div>
                    ...
                    <div class="d-flex flex-stack">
                        <div>
                            ...
                            <div class="fs-12 text-gray-700">
                                <div style="color: #d80000; font-weight: 500;">Data visi adalah blok</div>
                                <div style="color: #008c46 ; font-weight: 500;">Data visi adalah aktif</div>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('javascript')
    ...
    <script>
            ...
            var table = $('#example').DataTable({
                ...
                columns: [{
                ...
                },
                    {
                        data: "action",
                        render: function (data) {
                            var detail = '{{ route('admin.visi.show', [':visi']) }}';
                            var edit = '{{ route('admin.visi.edit', [':visi']) }}';
                            var x_edit = "";
                            var x_detail = "";
                            x_detail = `<a data-toggle='tooltip' data-placement='top' title='View' href='${detail.replace(':visi', data)}' class='btn btn-icon btn-bg-light btn-active-text-primary btn-sm me-1'><span class='bi bi-file-text ' aria-hidden='true'></span></a>`;
                            x_edit = `<a data-toggle='tooltip' data-placement='top' title='Edit' href='${edit.replace(':visi', data)}' class='btn btn-icon btn-bg-light btn-active-text-primary btn-sm me-1'><span class='bi bi-pencil ' aria-hidden='true'></span> </a>`;
        ...
        function deleteConfirmation(visi) {
            Swal.fire({
                ...
                if (result.value) {
                    var destroy = '{{ route('admin.visi.destroy', [':visi']) }}';
                    $.ajax({
                        url: destroy.replace(':visi', visi),
                        method: 'DELETE',
                        data: {
                            "visi": visi,
                            "_token": "{{ csrf_token() }}"
